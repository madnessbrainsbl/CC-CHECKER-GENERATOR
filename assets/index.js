$(document).ready(function () {
    ...
    $.ajax({
      ...
      success: function (data) {
        if (data && (data.brand || data.bank)) {
          var info = `${data.bank || "Unknown"}|${data.country || "??"}|${
            data.type || "Cc"
          }|${data.brand || "N/A"}`;
          callback(info);
        } else {
  ...
  function generateSingleCard(bin, month, year, cvvMode) {
    ...
    // Strict matrix: real patterns из dumps Dec 2025
    // Vary ТОЛЬКО last 7-8 digits для максимального realism
    var middlePatterns = [
      "",     // No middle (30% chance)
      "00",   // Common padding
      "10", "20", "50",  // Common increments
      "123",  // Sequential 3-digit
      "4567"  // Sequential 4-digit (most common в real dumps)
    ];
    
    var remainingLength = targetLength - cardNumber.length - 1; // -1 для checksum
    
    // 70% chance использовать middle pattern (strict matrix)
    ...
    // Fill remaining digits (tail - варьируем только последние 6-8 digits)
